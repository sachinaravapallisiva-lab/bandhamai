...
export const TERMS_PROFILE_DATA_BODY =
  "When you create or save a profile, Bandham stores the fields you send: name, gender, city, mother tongue, visa status, education, work, about you, what you want in a match, an Instagram username if you add one, a biodata share choice, and a profile photo. Name, gender, city, and a photo are required. The rest is optional. A reviewer must set the profile live before it appears on Browse. Do not put an Aadhaar number, passport scan, or other government ID in a profile or photo.";

export const TERMS_VERIFYAI_BIO_TITLE = "VerifyAI and biometrics on Bandham";
export const TERMS_VERIFYAI_BIO_BODY =
  "VerifyAI is optional. It costs $4.99 one time, billed by Stripe Checkout. Paying does not verify the profile. Bandham then hands you to the VerifyAI flow at verifyai.llc. That flow may ask for device biometrics on your phone. Bandham stores a payment row, a status (unverified, pending, verified, failed, or revoked), and an id from VerifyAI when one is returned. Bandham does not store a face map. A quiet badge appears only after VerifyAI succeeds and we have a stored verified status. It is not a background check, a visa check, or a promise that someone is who they say they are.";
