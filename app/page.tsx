...
export default function Home() {
  ...
  const [interested, setInterested] = useState<BrowseProfile[]>([]);
  ...
  return (
    <div style={{ minHeight: "100vh", background: WASH, color: INK }}>
      ...
      <main style={{ maxWidth: 640, margin: "0 auto", padding: "24px 20px 24px" }}>
        ...
        {tab === "browse" && (
          <>
            ...
            >
              ...
              <p className="bm-sans" style={{ margin: "0 0 10px", fontSize: 12, color: MUTED, lineHeight: 1.4 }}>
                {SEARCH_HINT}
              </p>
              ...
              {live ? (
                <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 3, height: 16, marginTop: 10 }}>
              ...
              <div className="bm-sans" style={{ minHeight: 18, marginTop: 8, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10 }}>
            ...
            {!searching && !currentProfile ? (
              <div style={{ background: CREAM, border: "1px solid " + LINE, borderRadius: 22, padding: "44px 22px", textAlign: "center" }}>
                <p className="bm-serif" style={{ margin: "0 0 7px", fontSize: 20 }}>
                  {emptyKind === "matches" ? "No matches for that yet." : loadedOnce && profiles.length === 0 ? "No live profiles yet." : "That's everyone for now."}
                </p>
                <p className="bm-sans" style={{ margin: 0, fontSize: 13.5, color: MUTED }}>
                  {emptyKind === "matches"
                    ...
                    : emptyKind === "inventory"
                      ...
                      : "Search again, or come back later."}
                </p>
              </div>
            ) : currentProfile ? (
              <div style={{ maxWidth: 440, margin: "0 auto" }}>
                <DiscoverCard
                  ...
                  interested={interested.some(function (x) { return x.id === currentProfile.id; })}
                  saved={savedIds.indexOf(currentProfile.id) >= 0}
                  signedIn={signedIn}
                  onInterested={function () { markInterested(currentProfile); }}
                  onPass={function () { passProfile(currentProfile.id); }}
                  onSave={function () { toggleSave(currentProfile.id); }}
                  onBlocked={function () {
                    const blockedId = currentProfile.id;
                    passProfile(blockedId);
                    setInterested(function (prev) {
                      return prev.filter(function (x) { return x.id !== blockedId; });
                    });
                    setSavedIds(function (prev) {
                      return prev.filter(function (id) { return id !== blockedId; });
                    });
                  }}
                />
              </div>
            ...
            {speedPartner ? (
              ...
              <div style={{ display: "grid", gap: 12 }}>
                {matches.map(function (p) {
                  return (
                    <article key={p.id} className="bm-card" style={{ background: CREAM, border: "1px solid " + LINE, borderRadius: 22, padding: "18px" }}>
                      ...
                      <p className="bm-sans" style={{ margin: "0 0 14px", fontSize: 13, color: MUTED }}>
                        {[p.work, p.city].filter(Boolean).join(" — ") || "Marked Interested on Browse"}
                      </p>
        ...
        {/* ---------------- CHAT ---------------- */}
        {tab === "chat" && (
          <div style={{ background: CREAM, border: "1px solid " + LINE, borderRadius: 22, overflow: "hidden" }}>
